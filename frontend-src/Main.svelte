<script>
  ...
  const loadNotes = ({ reset = false } = {}) => {
    ...
    return (fetching = getNotes({ age, search, page }).then((data) => {
      ...
      const uniqueIds = new Set(entries.map(entry => entry._id));


      const uniqueData = data.filter(item => !uniqueIds.has(item._id));

      entries = entries.concat(uniqueData);
...
<section class="uk-flex uk-grid-collapse">
  <aside class="uk-width-1-4 uk-padding-small">
    ...
    {#if entries.length > 0}
      ...
      {/each}
    {:else}
      <!-- Здесь можно добавить сообщение, если массив entries пустой -->
